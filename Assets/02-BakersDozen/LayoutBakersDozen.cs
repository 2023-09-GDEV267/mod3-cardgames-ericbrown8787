...
public class LayoutBakersDozen : MonoBehaviour
{
    ...
    public void ReadLayout(string xmlText)
    {
        ...
        for (int i = 0; i < slotsX.Count; i++)
        {
            ...
            {
                // If the slot attribute has a type attribute parse it
                tSD.type= slotsX[i].att("type");
            } else
            {
                // If not, set ist type to "slot":it's a card in the rows
                tSD.type = "slot";
            }
            ...
            switch (tSD.type)
            {
                // pull additional attributed based on the type of this <slot> 
                case "slot":
